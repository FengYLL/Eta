...
import io.github.fengyl.eta.agent.device.AgentNotificationHistoryService
...
internal data class AgentToolCapabilities(
    ...
    fun unavailableCode(name: String): String? {
        if (isolatedDisplay && !io.github.fengyl.eta.agent.display.DisplayToolPolicy.allows(name)) return "DISPLAY_SCOPE_REQUIRED"
        ...
        return JSONArray().also { visible ->
            for (index in 0 until rootProjected.length()) {
                ...
                if (unavailableCode(name) == null) visible.put(
                    if (isolatedDisplay) io.github.fengyl.eta.agent.display.DisplayToolPolicy.project(tool) else tool,
                )
            }
        }
    }
