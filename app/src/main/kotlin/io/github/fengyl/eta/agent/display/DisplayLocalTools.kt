...
import io.github.fengyl.eta.agent.accessibility.TextEditPlanner
...
import io.github.fengyl.eta.agent.runtime.AgentRunCancelledException
...
internal class DisplayLocalTools(
    ...
    private fun nodeAction(name: String, args: JSONObject): AgentModelClient.ToolResult {
        val obs = requireObservation(args)
        val selected = if (args.has("index")) {
            obs.tree.nodes.getOrNull(args.getInt("index"))
        } else {
            // Work-display apps may expose the input field without propagating
            // Android's input-focus bit because the ordinary IME is unavailable.
            DisplayNodeSelectionPolicy.select(obs.tree.nodes.map {
                DisplayNodeSelectionPolicy.Candidate(
                    index = it.index,
                    editable = it.value.isEditable,
                    focused = it.value.isFocused,
                )
            })?.let { index -> obs.tree.nodes.getOrNull(index) }
        }
