...
internal class DisplayLocalTools(
    ...
    private fun ensureSession() {
        if (acquired) return
        val accessibility = AgentAccessibilityKeeper.ensureEnabledForGuiOperation(context)
    ...
    override fun close() {
        ...
        synchronized(actionLock) {
            // Acquisition may have been in flight when close first revoked the lease.
            pauseBinding?.close()
            DisplaySessionStore.retain(run)
            clearObservation()
        }
    ...
    private fun executeLocked(call: AgentModelClient.ToolCall): AgentModelClient.ToolResult {
        return try {
            ...
            if (!acquired) return AgentModelClient.ToolResult(JSONObject().put("ok", false)
                .put("code", "WORK_DISPLAY_UNAVAILABLE")
                .put("message", "${failure.cause?.message ?: failure.message}。请打开工具能力中的工作屏进行检查；未在主屏执行。")
                .toString())
