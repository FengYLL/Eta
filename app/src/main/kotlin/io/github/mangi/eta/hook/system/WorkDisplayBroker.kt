...
import io.github.mangi.eta.agent.display.PreviewSurfaceLease
...
internal class WorkDisplayBroker(private val context: Context, private val loader: ClassLoader) {
    ...
    internal class Session(
        ...
        val preview = PreviewSurfaceLease(reader.surface,
            output = { display.surface = it }, release = { it.release() })
        var previewWatchdog: Runnable? = null
    }
    ...
    private fun dispatch(uid: Int, request: Bundle): Bundle {
        ...
        return synchronized(actionLock) {
            ...
            when (operation) {
                ...
                "preview_attach" -> {
                    val surface = request.getParcelable("surface", Surface::class.java) ?: error("缺少观看画面")
                    try {
                        check(surface.isValid) { "观看画面已销毁" }
                        validateEnvironment(session)
                        check(session.lease.state !in setOf(DisplayLease.State.HUMAN, DisplayLease.State.CLOSED, DisplayLease.State.LOST)) {
                            "工作屏不可观看，请关闭后重建"
                        }
                    } catch (failure: Throwable) { surface.release(); throw failure }
                    session.preview.attach(request.getLong("viewer"), surface, SystemClock.uptimeMillis())
                    status(session)
                }
                "preview_renew" -> {
                    check(session.preview.renew(request.getLong("viewer"), SystemClock.uptimeMillis())) { "观看连接已过期，请重新连接" }
                    status(session)
                }
                "preview_detach" -> {
                    // Detaching must work even while locked, paused, or after takeover.
                    session.preview.detach(request.getLong("viewer"))
                    status(session)
                }
                "launch" -> {
            ...
            val watchdog = object : Runnable {
                override fun run() {
                    synchronized(actionLock) {
                        if (sessions[uid] !== session) return
                        runCatching { session.preview.expire(SystemClock.uptimeMillis()) }.onFailure {
                            session.reason = "观看输出恢复失败，请关闭工作屏后重建"
                            session.lease.lost()
                            runCatching { destroy(session) }
                        }
                        if (sessions[uid] === session) worker.postDelayed(this, 1000)
                    }
                }
            }
            session.previewWatchdog = watchdog
            worker.postDelayed(watchdog, 1000)
            return status(session)
    ...
    private fun destroy(s: Session) {
        s.lease.close()
        s.previewWatchdog?.let(worker::removeCallbacks)
