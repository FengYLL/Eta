...
internal object DisplaySessionStore {
    ...
    private val viewerSequence = java.util.concurrent.atomic.AtomicLong()
    ...
    fun nextViewerId(): Long = viewerSequence.incrementAndGet()
    fun attachPreview(session: String, viewer: Long, surface: Surface): Snapshot = update(call("preview_attach", Bundle().apply {
        putLong("viewer", viewer); putParcelable("surface", surface)
    }, session))
    fun renewPreview(session: String, viewer: Long): Snapshot = update(call("preview_renew", Bundle().apply {
        putLong("viewer", viewer)
    }, session))
    fun detachPreview(session: String, viewer: Long) {
        if (broker == null || mutable.value.session != session) return
        update(call("preview_detach", Bundle().apply { putLong("viewer", viewer) }, session))
    }
    ...
    @Synchronized private fun update(result: Bundle): Snapshot {
        ...
        if (snapshot.epoch >= mutable.value.epoch && snapshot != mutable.value) {
            mutable.value = snapshot
    ...
    private fun call(op: String, extras: Bundle = Bundle(), expectedSession: String = mutable.value.session): Bundle {
        ...
        try {
            ...
            extras.putString("session", expectedSession)
