...
- **独立工作屏（实验性）**：开启后 GUI 任务自动创建虚拟副屏，复用当前用户已登录的应用；前台实时观看，返回聊天或切换其他应用后继续执行，从通知回到同一工作屏。需要 LSPosed 系统框架能力，支持暂停、停止并保留结果与主动接管。使用方法、隔离边界和真机验收见 [工作屏说明](docs/WORK_DISPLAY.md)。
